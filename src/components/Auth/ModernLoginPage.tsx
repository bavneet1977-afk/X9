...
const ModernLoginPage: React.FC<ModernLoginPageProps> = ({ 
  ...
  const [savedCredentials, setSavedCredentials] = useState<{[key: string]: {email: string, password: string}}>({});
  ...
  useEffect(() => {
    const saved = localStorage.getItem('attendify_saved_credentials');
    if (saved) {
      const credentials = JSON.parse(saved);
      setSavedCredentials(credentials);
      setShowSavedCredentials(Object.keys(credentials).length > 0);
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isRememberMe) {
      const credentialKey = `${formData.role}_${formData.email}`;
      const newSavedCredentials = {
        ...savedCredentials,
        [credentialKey]: {
      ...
      localStorage.setItem('attendify_saved_credentials', JSON.stringify(newSavedCredentials));
    }
  ...
  const useSavedCredential = (credentialKey: string) => {
    const credential = savedCredentials[credentialKey];
    if (credential) {
      setFormData({
        ...formData,
        email: credential.email,
        password: credential.password,
        role: credentialKey.split('_')[0]
      });
      setShowSavedCredentials(false);
    }
  };

  const removeSavedCredential = (credentialKey: string) => {
    const newSavedCredentials = { ...savedCredentials };
    delete newSavedCredentials[credentialKey];
    setSavedCredentials(newSavedCredentials);
    localStorage.setItem('attendify_saved_credentials', JSON.stringify(newSavedCredentials));
    setShowSavedCredentials(Object.keys(newSavedCredentials).length > 0);
  };
      ...
      <div className="relative z-10 w-full max-w-md animate-fade-in">
        ...
        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/30 dark:border-gray-700/30 p-8 hover:shadow-3xl transition-all duration-500 hover:scale-[1.02]">
          ...
          <div className="text-center mb-8 animate-slide-up">
            <div className="relative mb-6">
              <div className="relative mx-auto w-20 h-20">
                <img 
                  ...
                  alt="Attendify" 
                  className="w-20 h-20 rounded-2xl shadow-lg transition-transform duration-300 mx-auto relative z-10" 
                />
            ...
            <p className="text-gray-600 dark:text-gray-300 text-lg">{t('auth.sign_in')} to your Attendify account</p>
          ...
          {showSavedCredentials && Object.keys(savedCredentials).length > 0 && (
            <div className="mb-6 animate-fade-in">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
                {t('auth.saved_credentials')}
              </h3>
              <div className="space-y-2">
                {Object.entries(savedCredentials).map(([key, credential]) => {
                  const [role, email] = key.split('_');
                  const roleOption = roleOptions.find(r => r.id === role);
                  
                  return (
                    <div key={key} className="flex items-center justify-between p-3 bg-white/40 dark:bg-gray-700/40 backdrop-blur-sm rounded-xl border border-white/30 dark:border-gray-600/30">
                      <div className="flex items-center space-x-3">
                        {roleOption && (
                          <div className={`p-2 rounded-lg bg-gradient-to-r ${roleOption.color}`}>
                            <roleOption.icon className="w-4 h-4 text-white" />
                          </div>
                        )}
                        <div>
                      ...
                      <div className="flex items-center space-x-2">
                        <button
                          type="button"
                          onClick={() => useSavedCredential(key)}
                        ...
                        <button
                          type="button"
                          onClick={() => removeSavedCredential(key)}
          ...
          <form onSubmit={handleSubmit} className="space-y-6 animate-fade-in">
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-4 bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400">{t('auth.or_continue_with')}</span>
              </div>
            </div>
            ...
            <div className="text-center pt-6 border-t border-gray-200 dark:border-gray-700">
              <span className="text-gray-600 dark:text-gray-400">{t('auth.dont_have_account')} </span>
              <button
                type="button"
          ...
          <div className="mt-8 p-6 bg-gradient-to-r from-indigo-50/80 to-purple-50/80 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-2xl border border-indigo-100/50 dark:border-indigo-800/30 backdrop-blur-sm">
            <div className="flex items-center mb-3">
              ...
              <h4 className="text-sm font-semibold text-indigo-800 dark:text-indigo-300">{t('auth.demo_credentials')}</h4>
            </div>
            <div className="text-xs text-indigo-700 dark:text-indigo-400 space-y-2">
              ...
              <div className="flex justify-between items-center p-2 bg-white/50 dark:bg-gray-800/50 rounded-lg">
                <span className="font-medium">{t('auth.faculty')}:</span>
